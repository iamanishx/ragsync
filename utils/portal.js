...
const ALLOWED_PROVIDERS = new Set(['openrouter', 'groq', 'anthropic', 'google', 'openai']);
...
<style>body{font-family:system-ui;margin:2rem;max-width:560px}label{display:block;margin:.5rem 0}.note{color:#666;font-size:.9rem}input,select{padding:.5rem;width:100%;max-width:100%}</style>
  ...
  <p class="note">For: <b>${payload.userId}</b>${payload.guildId !== 'dm' ? ` in guild <b>${payload.guildId}</b>` : ''}</p>
  <form method="post" action="/setup">
    <input type="hidden" name="token" value="${String(token)}" />
    <label>Provider
      <select name="provider">
        ${['openrouter','groq','anthropic','google','openai'].map(p => `<option value="${p}" ${p===provider?'selected':''}>${p}</option>`).join('')}
      </select>
    </label>
    <label>API Key
  ...
  app.post('/setup', async (req, res) => {
    try {
      ...
      if (typeof apiKey !== 'string' || apiKey.length < 8) throw new Error('Invalid API key');

      const keyName = payload.scope === 'guild'
